	...
	public class CharacterLadder : CharacterAbility 
	{
		...
		protected virtual void HandleLadderClimbing()
		{
			...
			if (LadderColliding) 
			{
				...
				if (_inputManager.PrimaryMovement.y > _inputManager.Threshold.y// if the player is pressing up
				...
				{
					if ((_movement.CurrentState != CharacterStates.MovementStates.LadderClimbing) // if the character is climbing
						&& (_inputManager.PrimaryMovement.y < -_inputManager.Threshold.y) // and is pressing down
		...
		protected virtual void Climbing()
		{
			...
			{
				_controller.SetVerticalForce(_inputManager.PrimaryMovement.y * LadderClimbingSpeed);
				// we set the climbing speed state.
				CurrentLadderClimbingSpeed=Mathf.Abs(_inputManager.PrimaryMovement.y ) * Vector2.up;	
			}
			if (CurrentLadder.LadderType == Ladder.LadderTypes.BiDirectional)
			{
				_controller.SetHorizontalForce(_inputManager.PrimaryMovement.x * LadderClimbingSpeed);
				_controller.SetVerticalForce(_inputManager.PrimaryMovement.y * LadderClimbingSpeed);
				CurrentLadderClimbingSpeed = Mathf.Abs(_inputManager.PrimaryMovement.x ) * Vector2.right;	
				CurrentLadderClimbingSpeed += Mathf.Abs(_inputManager.PrimaryMovement.y ) * Vector2.up;	
			}
		...
		/// </summary>
		protected virtual void GetOffTheLadder()
		{
		...
		protected virtual bool AboveLadderPlatform()
		{
			...
			{
				colliderY = CurrentLadder.LadderPlatformBoxCollider2D.bounds.center.y + CurrentLadder.LadderPlatformBoxCollider2D.bounds.extents.y ;
			}
			if (CurrentLadder.LadderPlatformEdgeCollider2D != null)
			{
				colliderY = CurrentLadder.LadderPlatform.transform.position.y 
			...
			if (colliderY < _controller.ColliderBottomPosition.y + _ladderTopSkinHeight)
			{
				return true;
			}
			else
